...
# Try to load images, use colored squares if not found
x_image = load_image('x.png', 0.8)
o_image = load_image('o.png', 0.8)
...
def handle_login_events(event):
    ...
    global username, password, active_input, game_state, message, current_user
    ...
    if event.type == pygame.KEYDOWN:
        ...
        x, y = event.pos
        # Check if clicked on username or password fields
        if SCREEN_WIDTH // 2 - 130 <= x <= SCREEN_WIDTH // 2 + 130:
            if 290 <= y <= 330:
                active_input = "username"
            elif 370 <= y <= 410:
...
while running:
    ...
    if game_state == LOGIN:
        ...
        buttons = [login_button, about_button]
    elif game_state == MENU:
        for button in buttons:
            buttons = button.check_hover(mouse_pos)
